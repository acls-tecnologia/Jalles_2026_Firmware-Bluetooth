...
static tanque_limits_cache_t g_limits = {.min_pct = 0, .max_pct = 100, .last_ms = 0, .valid = false};
...
static bool api_fetch_tanque_limits(int *out_min,
                                    int *out_max); // Protótipo da função de obtenção de limites do tanque via API
static void
api_get_tanque_limits_cached(int *out_min,
                             int *out_max); // Protótipo da função de obtenção de limites do tanque com cache
void nivel_task(void *pv);                  // Protótipo da task de nível
static esp_err_t mcp_escrever_bomba_idx(int idx,
                                        bool ligar); // Protótipo da função de escrita da bomba para múltiplas bombas
void bombas_task(void *pv);                          // Protótipo da task de controle da bomba
static int
bomba_index_from_id(uint16_t bomba_id); // Protótipo da função para obter o índice da bomba a partir do ID da bomba
static float map_4_20_to_range(float ma, float out_min,
                               float out_max); // Protótipo da função de mapeamento 4-20mA para faixa
static bool
ler_corrente_bomba_idx(int idx,
                       float *out_corrente_a); // Protótipo da função de leitura da corrente da bomba por índice
...
bool tratar_comando_bomba(const char *cmd, int valor, uint16_t bomba_id,
                          int controle_id); // Protótipo da função de tratamento de comando da bomba recebido via LoRa
...
static bool is_relay_target(uint16_t id) { return (id == 28); }
...
static bool should_relay_frame(const lora_app_frame_t *rx, uint16_t my_id) {
    ...
    if (rx->src_type == DEV_TANK && is_relay_target(rx->src_id) && rx->dst_type == DEV_GTW) {
        return true;
    }
...
static bool text_has_value(const char *value) { return value && value[0] != '\0'; }
...
void app_main(void) {
    ...
    ESP_LOGW("MAIN", "CFG ATUAL: prov=%d perfil=%d unidade=%d tanque_id=%d qtd=%d b0=%d b1=%d b2=%d nome=%s",
             (int)g_cfg.provisionado, (int)g_cfg.perfil, g_cfg.unidade_id, g_cfg.tanque_id, g_cfg.qtd_bombas,
             g_cfg.bomba_id[0], g_cfg.bomba_id[1], g_cfg.bomba_id[2], g_cfg.nome_tanque);
...
esp_err_t Port_config(i2c_master_bus_handle_t *bus_handle, i2c_master_dev_handle_t *mcp_handle,
                      i2c_master_dev_handle_t *ads_handle, bool enable_mcp) {
    ...
    if (bus_handle == NULL) {
        ...
        ESP_LOGE(TAG_Port_config, " Contador de glitch inválido: %d. Deve ser entre 0 e 15.",
                 I2c_Config.glitch_ignore_cnt);
        return ESP_ERR_INVALID_ARG;
    } else {
        ESP_LOGW(TAG_Port_config, " Configuração do barramento I2C: Porta %d, SDA %d, SCL %d, Glitch Ignore Count %d",
                 I2c_Config.i2c_port, I2c_Config.sda_io_num, I2c_Config.scl_io_num, I2c_Config.glitch_ignore_cnt);
    ...
    while (retry_count < MAX_RETRIES_CONFIG) {
        ...
        if (ret == ESP_OK) {
            ESP_LOGI(TAG_Port_config, " >>> I2C master configurado com sucesso (tentativa %d/%d)", retry_count + 1,
                     MAX_RETRIES_CONFIG);
            i2c_ok = true;
            break;
        }

        ESP_LOGE(TAG_Port_config, " >>> Falha na configuração do I2C (tentativa %d/%d): %s", retry_count + 1,
                 MAX_RETRIES_CONFIG, esp_err_to_name(ret));
        vTaskDelay(pdMS_TO_TICKS(2000));
        retry_count++;
    }

    if (!i2c_ok) {
        ESP_LOGE(TAG_Port_config, " >>> ERRO FATAL: Não foi possível configurar o barramento I2C após %d tentativas.",
                 MAX_RETRIES_CONFIG);
    ...
    if (enable_mcp) {
        ...
        while (retry_count < MAX_RETRIES_CONFIG) {
            ...
            if (ret == ESP_OK) {
                mcp_add_ok = true;
                ESP_LOGI(TAG_Port_config, " >>> MCP23017 adicionado com sucesso ao barramento (tentativa %d/%d)",
                         retry_count + 1, MAX_RETRIES_CONFIG);
                break;
            }

            ESP_LOGE(TAG_Port_config, " >>> Falha ao adicionar MCP23017 (tentativa %d/%d): %s", retry_count + 1,
                     MAX_RETRIES_CONFIG, esp_err_to_name(ret));
        ...
        if (!mcp_add_ok) {
            ESP_LOGE(TAG_Port_config, " >>> ERRO: MCP23017 não pôde ser adicionado após %d tentativas.",
                     MAX_RETRIES_CONFIG);
            goto resumo;
        }
        ...
        while (retry_count < MAX_RETRIES_CONFIG) {
            ...
            if (mcp_status == 0) {
                mcp_init_ok = true;
                ESP_LOGI(TAG_Port_config, " >>> MCP23017 inicializado com sucesso (tentativa %d/%d)", retry_count + 1,
                         MAX_RETRIES_CONFIG);
                break;
            }
        ...
        if (!mcp_init_ok) {
            ESP_LOGE(TAG_Port_config, " ERRO: MCP23017 NÃO foi inicializado corretamente após %d tentativas.",
                     MAX_RETRIES_CONFIG);
            goto resumo;
        }
    } else {
    ...
    while (retry_count < MAX_RETRIES_CONFIG) {
        ...
        if (ret == ESP_OK) {
            ads_dev_ok = true;
            ESP_LOGI(TAG_Port_config, " ADS1115 adicionado com sucesso (tentativa %d/%d)", retry_count + 1,
                     MAX_RETRIES_CONFIG);
            break;
        }

        ESP_LOGE(TAG_Port_config, "Falha ao adicionar ADS1115 (tentativa %d/%d): %s", retry_count + 1,
                 MAX_RETRIES_CONFIG, esp_err_to_name(ret));
...
void wifi_task(void *pv) {
    ...
    while (1) {
        ...
        if (!wifi_driver_ok) {
            if ((now_ms - last_driver_retry_ms) >= 5000) {
                ...
                if (start_err == ESP_OK) {
                    ...
                    ESP_LOGE(TAG_WIFI, "Falha ao iniciar Wi-Fi driver: %s", esp_err_to_name(start_err));
                }
            }
        ...
        if (!sta_ok || !ip_ok) {
            ...
            int retry_ms = first_online_done ? WIFI_RUNTIME_RETRY_INTERVAL_MS : WIFI_BOOT_RETRY_INTERVAL_MS;
            ...
            if ((now_ms - last_wifi_try_ms) >= retry_ms) {
                ...
                esp_err_t err = wifi_connect_credentials(wifi_ssid, wifi_password, WIFI_CONNECT_TIMEOUT_MS);
                ...
                if (err == ESP_OK) {
                    ...
                    ESP_LOGW(TAG_WIFI, "Falha ao conectar Wi-Fi (%d/%d): %s", wifi_connect_fail_count,
                             WIFI_CONNECT_ATTEMPTS_BEFORE_SLEEP, esp_err_to_name(err));
                    ...
                    if (wifi_connect_fail_count >= WIFI_CONNECT_ATTEMPTS_BEFORE_SLEEP) {
                        ...
                        ESP_LOGW(TAG_WIFI, "Wi-Fi pausado por %u minutos apos falhas de conexao",
        ...
        // =========================================================
        int check_interval_ms = first_online_done ? INTERNET_CHECK_INTERVAL_MS : INTERNET_BOOT_CHECK_INTERVAL_MS;
        ...
        if ((now_ms - last_net_check_ms) >= check_interval_ms) {
            ...
            bool internet_ok = wifi_check_internet_tcp(INTERNET_HEALTH_URL, INTERNET_TIMEOUT_MS);
            ...
            if (internet_ok) {
                ...
                ESP_LOGW(TAG_WIFI, "Sem internet/backend (%d/%d) -> LoRa assume", fails_internet,
...
void ler_portas_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (xSemaphoreTake(i2c_semaphore, pdMS_TO_TICKS(500)) == pdTRUE) {
            ...
            ESP_LOGE(TAG, "Timeout no semáforo do I2C (holder=%s)", holder ? pcTaskGetName(holder) : "NULL");
            vTaskDelay(pdMS_TO_TICKS(1000));
            continue;
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (last_modo_bomba[i] == -1) {
                ...
                ESP_LOGW(TAG, "Modo bomba %d mudou: %d -> %d", i + 1, last_modo_bomba[i], modo_bomba);
                ...
                if (modo_bomba == 0) // entrou em manual/local
                {
                    ...
                    ESP_LOGW(TAG, "Bomba %d entrou em MANUAL/LOCAL: comando interno bloqueado", i + 1);

                    if (bomba_estava_ativa) {
                        enviar_alerta_bomba_api_online(i, "BOMBA PARADA", "modo manual/local acionado");
                    }
        ...
        if (last_emergencia_global == -1) {
            ...
            ESP_LOGW(TAG, "Emergência GLOBAL mudou: %d -> %d", last_emergencia_global, emergencia_global_atual);
            ...
            if (emergencia_global_ativa) {
                ...
                for (int j = 0; j < n; j++) {
                    ...
                    if (bomba_estava_ativa) {
                        enviar_alerta_bomba_api_online(j, "BOMBA PARADA", "emergencia global acionada");
                    }

                    if (net_ok && auth_ok) {
                        if (xSemaphoreTake(MutexHTTP, pdMS_TO_TICKS(3000)) == pdTRUE) {
                            (void)api_patch_bomba_status(g_cfg.bomba_id[j], false);
                            ESP_LOGW(TAG, "EMERGÊNCIA GLOBAL: PATCH HTTP bomba_id=%d status=0", g_cfg.bomba_id[j]);
                            xSemaphoreGive(MutexHTTP);
                        } else {
                            ESP_LOGW(TAG, "EMERGÊNCIA GLOBAL: falha ao obter MutexHTTP");
                        }
                    } else {
                        bool ok = lora_enqueue_json_bomba_int((uint16_t)g_cfg.bomba_id[j], "status", 0);

                        ESP_LOGW(TAG, "EMERGÊNCIA GLOBAL: LoRa enqueue bomba_id=%d status=0 -> %s", g_cfg.bomba_id[j],
                                 ok == pdTRUE ? "OK" : "FAIL");
                    }
                }
            } else {
        ...
        for (int i = 0; i < g_cfg.qtd_bombas && i < MAX_BOMBAS; i++) {
            if (boot_status_pendente[i] && boot_status_due_ms[i] > 0 && now_boot_status_ms >= boot_status_due_ms[i]) {
                patch_valor[i] = g_status_bomba[i] ? 1 : 0;
                patch_pendente[i] = true;
                boot_status_pendente[i] = false;

                ESP_LOGI(TAG, "Sync final boot status bomba %d id=%d valor=%d", i + 1, g_cfg.bomba_id[i],
                         patch_valor[i]);
            }
        }
        ...
        // ----------- ENVIO PATCH STATUS POR BOMBA (somente a bomba que mudou) -----------
        {
            ...
            if (status_periodic_offset_ms < 0) {
                ...
                status_periodic_offset_ms = (int64_t)((slot_id % 20U) * BOMBA_STATUS_PERIODIC_SLOT_MS) +
                                            (int64_t)(esp_random() % BOMBA_STATUS_PERIODIC_JITTER_MS);

                ESP_LOGI(TAG, "Envio periodico status bomba: base=%u min offset=%lld ms",
                         (unsigned)(BOMBA_STATUS_PERIODIC_SEND_MS / 60000ULL), (long long)status_periodic_offset_ms);
            }
            ...
            if (api_ready && !api_ready_prev) {
                for (int i = 0; i < g_cfg.qtd_bombas && i < MAX_BOMBAS; i++) {
                    ...
                    ESP_LOGI(TAG, "API online/auth OK: agendando sync status atual bomba %d id=%d valor=%d", i + 1,
                             g_cfg.bomba_id[i], patch_valor[i]);
                }

                last_status_periodic_ms = now_status_ms;
            } else if (api_ready && last_status_periodic_ms > 0 &&
                ...
                for (int i = 0; i < g_cfg.qtd_bombas && i < MAX_BOMBAS; i++) {
                    ...
                    ESP_LOGI(TAG, "Sync periodico API status bomba %d id=%d valor=%d", i + 1, g_cfg.bomba_id[i],
                             patch_valor[i]);
                }
            ...
            if (tem_pendente) {
                if (net_ok && auth_ok) {
                    ...
                    if (xSemaphoreTake(MutexHTTP, pdMS_TO_TICKS(3000)) == pdTRUE) {
                        for (int i = 0; i < g_cfg.qtd_bombas && i < MAX_BOMBAS; i++) {
                            ...
                            if (e == ESP_OK) {
                                ...
                                ESP_LOGE(TAG, "PATCH status bomba %d falhou (%s) - vai tentar de novo", i + 1,
                                         esp_err_to_name(e));

                                // Se falhou por problema típico de conectividade,
                                // envia imediatamente via LoRa para não perder a transição.
                                if (e == ESP_ERR_HTTP_CONNECT || e == ESP_ERR_HTTP_EAGAIN || e == ESP_ERR_TIMEOUT) {
                                    bool ok = lora_enqueue_json_bomba_int((uint16_t)g_cfg.bomba_id[i], "status",
                                                                          patch_valor[i] ? 1 : 0);

                                    if (ok) {
                                        ESP_LOGW(TAG, "HTTP falhou -> status bomba %d enviado via LoRa: S%d", i + 1,
                                                 patch_valor[i] ? 1 : 0);
                    ...
                    for (int i = 0; i < g_cfg.qtd_bombas && i < MAX_BOMBAS; i++) {
                        ...
                        bool ok =
                            lora_enqueue_json_bomba_int((uint16_t)g_cfg.bomba_id[i], "status", patch_valor[i] ? 1 : 0);

                        if (ok) {
                            ESP_LOGI(TAG, "LoRa status bomba %d enviado: {\"status\":%d}", i + 1,
                                     patch_valor[i] ? 1 : 0);
            ...
            if ((bits & SYS_NET_ONLINE_BIT) && (bits & SYS_AUTH_OK_BIT)) {
                if (xSemaphoreTake(MutexHTTP, pdMS_TO_TICKS(3000)) == pdTRUE) {
                    ...
                    if (err == ESP_OK) {
                        ...
                        ESP_LOGE(TAG, "Falha ao enviar ALERTA (%s). Vai tentar de novo.", esp_err_to_name(err));
            ...
            if (net_ok && auth_ok) {
                if (xSemaphoreTake(MutexHTTP, pdMS_TO_TICKS(3000)) == pdTRUE) {
                    ...
                    if (err == ESP_OK) {
                        ...
                        ESP_LOGE(TAG, "Falha ao enviar alerta MANUAL da bomba %d (%s)", i + 1, esp_err_to_name(err));
                    }
                }
            } else {
                bool ok = lora_enqueue_json_bomba_int((uint16_t)g_cfg.bomba_id[i], "automatico", 0);
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (net_ok && auth_ok) {
                ...
                if (xSemaphoreTake(MutexHTTP, pdMS_TO_TICKS(3000)) == pdTRUE) {
                    ...
                    if (err == ESP_OK) {
                        ...
                        ESP_LOGI(TAG, "PATCH modo bomba %d OK (bomba_id=%d, automatico=%d)", i + 1, g_cfg.bomba_id[i],
                                 last_modo_bomba[i]);
                    } else {
                        ESP_LOGE(TAG, "PATCH modo bomba %d falhou (%s)", i + 1, esp_err_to_name(err));

                        // se falhou por conectividade, cai para LoRa também
                        if (err == ESP_ERR_HTTP_CONNECT || err == ESP_ERR_HTTP_EAGAIN || err == ESP_ERR_TIMEOUT) {
                            bool ok = lora_enqueue_json_bomba_int((uint16_t)g_cfg.bomba_id[i], "automatico",
                                                                  last_modo_bomba[i] ? 1 : 0);

                            if (ok) {
                                last_modo_bomba_lora_enviado[i] = last_modo_bomba[i];

                                ESP_LOGW(TAG, "HTTP falhou -> modo bomba %d enviado via LoRa: A%d", i + 1,
                                         last_modo_bomba[i] ? 1 : 0);
                            } else {
                                ESP_LOGW(TAG, "HTTP falhou e fila LoRa cheia ao enviar modo bomba %d", i + 1);
                            }
                        }
                    }
                }
            ...
            // ==========================
                ...
                bool ok =
                    lora_enqueue_json_bomba_int((uint16_t)g_cfg.bomba_id[i], "automatico", last_modo_bomba[i] ? 1 : 0);
                if (ok) {
                    last_modo_bomba_lora_enviado[i] = last_modo_bomba[i];

                    ESP_LOGI(TAG, "LoRa modo bomba %d enviado: A%d", i + 1, last_modo_bomba[i] ? 1 : 0);
...
void nivel_task(void *pv) {
    ...
    while (1) {
        ...
        if (nivel_periodic_offset_ms < 0) {
            ...
            nivel_periodic_offset_ms =
                (int64_t)((slot_id % 20) * NIVEL_PERIODIC_SLOT_MS) + (int64_t)(esp_random() % NIVEL_PERIODIC_JITTER_MS);
            nivel2_periodic_offset_ms = nivel_periodic_offset_ms + 1500;
            ESP_LOGI(TAG, "Envio periodico nivel: base=%u min offset=%lld ms",
                     (unsigned)(NIVEL_PERIODIC_SEND_MS / 60000ULL), (long long)nivel_periodic_offset_ms);
        }
        ...
        if (pode_http) {
                ...
                if (xSemaphoreTake(MutexHTTP, pdMS_TO_TICKS(3000)) == pdTRUE) {
                    ...
                    if (patch) {
                        ...
                        if (err == ESP_OK) {
                            last_sent_pct = pct;
                            last_send_ms = now_ms;
                            ESP_LOGI(TAG, "Nivel atualizado na API: %d%% (v=%.3fV, i=%.2fmA, motivo=%s)", pct, v, ma,
                                     envio_periodico ? "periodico" : "mudanca");
                        } else {
            ...
            if (min_interval_ok && deve_enviar_nivel) {
                ...
                ESP_LOGI("LORA", "enqueue NIVEL pct=%d motivo=%s -> %s", pct, envio_periodico ? "periodico" : "mudanca",
                         ok ? "OK" : "FAIL");
        ...
        if (pode_http) {
            ...
            if (!alert_min_sent && pct <= lim_min) {
                if (xSemaphoreTake(MutexHTTP, pdMS_TO_TICKS(3000)) == pdTRUE) {
                    char msg[160];
                    snprintf(msg, sizeof(msg), "ALERTA: NÍVEL MÍNIMO ATINGIDO %s", NOME_CURTO);
            ...
            if (!alert_max_sent && pct >= lim_max) {
                if (xSemaphoreTake(MutexHTTP, pdMS_TO_TICKS(3000)) == pdTRUE) {
                    char msg[160];
                    snprintf(msg, sizeof(msg), "ALERTA: NÍVEL MÁXIMO ATINGIDO %s", NOME_CURTO);
            ...
            if (!alert_min_sent && pct <= lim_min) {
                ...
                ESP_LOGW("LORA", "enqueue ALERTA MIN codigo=6 pct=%d -> %s", pct, ok ? "OK" : "FAIL");
            ...
            if (!alert_max_sent && pct >= lim_max) {
                ...
                ESP_LOGW("LORA", "enqueue ALERTA MAX codigo=7 pct=%d -> %s", pct, ok ? "OK" : "FAIL");
        ...
        if (tanque_especial_2niveis_ativo()) {
            ...
            if (v2 >= 0.0f) {
                ...
                bool envio_periodico_n2 =
                    (last_send_ms_n2 > 0 && (uint64_t)(now_ms - last_send_ms_n2) >= periodic_delay_n2_ms);
                ...
                if (min_interval_ok_n2 && deve_enviar_nivel2) {
                    if (pode_http) {
                        ...
                        bool ok2 =
                            lora_enqueue_json_tanque_id_int((uint16_t)TANQUE_ESPECIAL_NIVEL2_ID, "nivel_atual", pct2);

                        ESP_LOGI("LORA", "enqueue NIVEL2 pct=%d motivo=%s -> %s", pct2,
                                 envio_periodico_n2 ? "periodico" : "mudanca", ok2 ? "OK" : "FAIL");
                ...
                if (pode_http) {
                    if (!alert_min_sent_n2 && pct2 <= lim_min) {
                        if (xSemaphoreTake(MutexHTTP, pdMS_TO_TICKS(3000)) == pdTRUE) {
                            char msg[160];
                            snprintf(msg, sizeof(msg), "ALERTA: NÍVEL MÍNIMO ATINGIDO %s NIVEL 2", NOME_CURTO);
                    ...
                    if (!alert_max_sent_n2 && pct2 >= lim_max - 1) {
                        if (xSemaphoreTake(MutexHTTP, pdMS_TO_TICKS(3000)) == pdTRUE) {
                            char msg[160];
                            snprintf(msg, sizeof(msg), "ALERTA: NÍVEL MÁXIMO ATINGIDO %s NIVEL 2", NOME_CURTO);
                    ...
                    if (!alert_min_sent_n2 && pct2 <= lim_min) {
                        ...
                        ESP_LOGW("LORA", "enqueue ALERTA MIN NIVEL2 codigo=8 pct=%d -> %s", pct2, ok ? "OK" : "FAIL");
                    ...
                    if (!alert_max_sent_n2 && pct2 >= lim_max - 1) {
                        ...
                        ESP_LOGW("LORA", "enqueue ALERTA MAX NIVEL2 codigo=9 pct=%d -> %s", pct2, ok ? "OK" : "FAIL");
        ...
        for (int i = 0; i < g_cfg.qtd_bombas; i++) {
            ...
            if (aguardando[i]) {
                ...
                if (real_atual == desejado) {
                    ESP_LOGI(TAG_BOMBA, "[B%d id=%d] Confirmado pelo retorno real: %s", i + 1, g_cfg.bomba_id[i],
                             desejado ? "LIGADA" : "DESLIGADA");

                    aguardando[i] = false;
                } else {
                    int64_t now = esp_timer_get_time() / 1000;
                    if (now - t0_ms[i] > 30000) {
                        ESP_LOGE(TAG_BOMBA, "[B%d id=%d] Timeout confirmacao 30s (real=%d desejado=%d)", i + 1,
                                 g_cfg.bomba_id[i], real_atual ? 1 : 0, desejado ? 1 : 0);
                        enviar_alerta_bomba_api_online(i, desejado ? "FALHA AO LIGAR BOMBA" : "FALHA AO DESLIGAR BOMBA",
...
static bool ler_corrente_bomba_idx(int idx, float *out_corrente_a) {
    ...
    if (xSemaphoreTake(i2c_semaphore, pdMS_TO_TICKS(1200)) != pdTRUE) {
        TaskHandle_t holder = xSemaphoreGetMutexHolder(i2c_semaphore);
        ESP_LOGE(TAGC, "Falha ao obter I2C para corrente bomba %d (holder=%s)", idx + 1,
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (v < 0.0f) {
            ESP_LOGW(TAGC, "Leitura ADS inválida na bomba %d (amostra %d/%d)", idx + 1, i + 1, N);
            xSemaphoreGive(i2c_semaphore);
            return false;
        }
...
void corrente_task(void *pv) {
    ...
    while (1) {
        ...
        for (int i = 0; i < g_cfg.qtd_bombas && i < MAX_BOMBAS; i++) {
            ...
            // =========================================================
            if (!bomba_ligada) {
                bool precisa_zerar = (last_bomba_ligada_sync[i] != false) || (last_corrente[i] != 0.0f);
                ...
                if (xSemaphoreTake(MutexHTTP, pdMS_TO_TICKS(3000)) == pdTRUE) {
                    ...
                    if (err == ESP_OK) {
                        ...
                        ESP_LOGW(TAGC, "Falha ao zerar corrente da bomba %d (%s)", i + 1, esp_err_to_name(err));
                    }
                }
            ...
            bool mudou_corrente =
                (last_corrente[i] < 0.0f) || (fabsf(corrente_a - last_corrente[i]) >= CORRENTE_DEADBAND_A);
            ...
            bool deve_enviar = mudou_estado || (mudou_corrente && intervalo_ok);
            ...
            if (xSemaphoreTake(MutexHTTP, pdMS_TO_TICKS(3000)) == pdTRUE) {
                ...
                if (err == ESP_OK) {
                    ...
                    ESP_LOGI(TAGC, "PATCH corrente bomba %d OK -> %d A", i + 1, corrente_i);
                } else {
                    ESP_LOGW(TAGC, "Falha PATCH corrente bomba %d (%s)", i + 1, esp_err_to_name(err));
                }
            }
        }
...
static void tanque_lora_log_rx_stats(const tanque_lora_rx_stats_t *s, const char *motivo) {
    ...
    ESP_LOGW("LORA_RX_STATS",
             ...
             (unsigned long)s->bad_preamble, (unsigned long)s->bad_crc, (unsigned long)s->duplicate,
             (unsigned long)s->wrong_dst, (unsigned long)s->ack_rx, (unsigned long)s->ack_match,
             (unsigned long)s->ack_unexpected, (unsigned long)s->ack_sent_for_payload, (unsigned long)s->app_queue_ok,
...
// Envia uma mensagem para a GTW via LoRa, esperando ACK se configurado. Retorna true se ACK recebido ou se não requer
// ACK.
bool tanque_send_to_gtw_ack(uint16_t src_tank_id, uint16_t gtw_id, const char *msg, uint8_t has_bomba,
                            uint16_t bomba_id) {
...
// Task para enviar mensagens do tanque para a GTW via LoRa, lendo de uma fila. Cada item da fila contém a mensagem e
// metadados.
...
void tanque_lora_rx_task(void *pv) {
    ...
    while (1) {
        ...
        ESP_LOGI("LORA",
                 ...
                 (unsigned long)stats.frame_ok, r, rx.src_type, rx.src_id, rx.dst_type, rx.dst_id, rx.msg_type,
                 rx.msg_id, rx.len);

        size_t expected_air_len = lora_frame_air_len(&rx);
        if (expected_air_len == 0 || r != (int)expected_air_len) {
            stats.incomplete++;
            ESP_LOGW("LORA", "Frame incompleto recebido: %d esperado=%u", r, (unsigned)expected_air_len);
        ...
        if (!lora_received_frame_valid(&rx, r, &crc_calc, &crc_rx)) {
            stats.bad_crc++;
            ESP_LOGW("LORA", "CRC invalido msg_id=%u recebido=0x%04X calculado=0x%04X", rx.msg_id, crc_rx, crc_calc);
        ...
        if (lora_is_duplicate(&rx)) {
            stats.duplicate++;
            ESP_LOGW("LORA", "Frame duplicado ignorado -> src=%u type=%u msg_id=%u", rx.src_id, rx.msg_type, rx.msg_id);
        ...
        if (rx.dst_type != DEV_TANK || rx.dst_id != my_id) {
            stats.wrong_dst++;
            ESP_LOGW("LORA", "Destino invalido -> dst_type=%u dst_id=%u meu_id=%u", rx.dst_type, rx.dst_id, my_id);
...
static void lora_boot_snapshot_task(void *pv) {
    ...
    if (g_cfg.tanque_id > 0) {
        ...
        if (ler_nivel_atual_pct(&nivel_pct)) {
            bool ok = lora_enqueue_json_tanque_id_int((uint16_t)g_cfg.tanque_id, "nivel_atual", nivel_pct);
...
bool sincronizar_status_bomba_real(int idx) {
    if (idx < 0 || idx >= g_cfg.qtd_bombas || idx >= MAX_BOMBAS || g_cfg.bomba_id[idx] <= 0) {
        ESP_LOGW(TAG_BOMBA, "Sync status real ignorado: idx=%d qtd=%d bomba_id=%d", idx, g_cfg.qtd_bombas,
                 (idx >= 0 && idx < MAX_BOMBAS) ? g_cfg.bomba_id[idx] : -1);
        return false;
    }
    ...
    if (net_ok && auth_ok) {
        if (xSemaphoreTake(MutexHTTP, pdMS_TO_TICKS(3000)) == pdTRUE) {
            ...
            if (err == ESP_OK) {
                ESP_LOGI(TAG_BOMBA, "Sync status real via HTTP OK bomba_id=%d status=%d", g_cfg.bomba_id[idx],
                         real_ligada ? 1 : 0);
                return true;
            }
        ...
        ESP_LOGW(TAG_BOMBA, "Sync status real sem internet/auth (net=%d auth=%d); enviando via LoRa", net_ok ? 1 : 0,
                 auth_ok ? 1 : 0);
    }

    bool ok = lora_enqueue_json_bomba_int((uint16_t)g_cfg.bomba_id[idx], "status", real_ligada ? 1 : 0);
    ESP_LOGW(TAG_BOMBA, "Sync status real via LoRa bomba_id=%d status=%d -> %s", g_cfg.bomba_id[idx],
             real_ligada ? 1 : 0, ok ? "OK" : "FAIL");
    return ok;
}
...
static bool alerta_bomba_reseta_controle(uint8_t codigo) { return codigo == 3 || codigo == 4 || codigo == 5; }

bool enviar_alerta_bomba_api_online(int idx, const char *evento, const char *motivo) {
    if (idx < 0 || idx >= g_cfg.qtd_bombas || idx >= MAX_BOMBAS || g_cfg.bomba_id[idx] <= 0) {
        ESP_LOGW(TAG_BOMBA, "Alerta bomba API ignorado: idx=%d qtd=%d bomba_id=%d", idx, g_cfg.qtd_bombas,
                 (idx >= 0 && idx < MAX_BOMBAS) ? g_cfg.bomba_id[idx] : -1);
        return false;
    }
    ...
    if (!net_ok || !auth_ok) {
        ...
        ESP_LOGW(TAG_BOMBA,
                 "Alerta bomba via LoRa codigo=%u net=%d auth=%d bomba_id=%d controle_id=%d -> alerta=%s ctrl=%s",
                 codigo, net_ok ? 1 : 0, auth_ok ? 1 : 0, g_cfg.bomba_id[idx], controle_id, ok ? "OK" : "FAIL",
                 ctrl_ok ? "OK" : "FAIL");
        return ok && ctrl_ok;
    }
    ...
    char msg[160];
    if (evento && strcmp(evento, "BOMBA PARADA") == 0) {
        snprintf(msg, sizeof(msg), "ALERTA BOMBA PARADA: Tanque %s, Bomba %d, %s", tanque_nome, g_cfg.bomba_id[idx],
                 motivo_msg);
    } else {
        snprintf(msg, sizeof(msg), "ALERTA BOMBA: Tanque %s, Bomba %d, %s, %s", tanque_nome, g_cfg.bomba_id[idx],
                 evento ? evento : "evento", motivo_msg);
    }
    ...
    if (xSemaphoreTake(MutexHTTP, pdMS_TO_TICKS(3000)) == pdTRUE) {
        ...
        if (codigo != 0 &&
            ...
            ESP_LOGW(TAG_BOMBA,
                     "MutexHTTP ocupado; alerta bomba enviado via LoRa codigo=%u bomba_id=%d controle_id=%d ctrl=%s",
    ...
    ESP_LOGW(TAG_BOMBA, "Falha alerta bomba API (%s) ctrl=%s: %s", esp_err_to_name(err), esp_err_to_name(ctrl_err),
             msg);
    return false;
}
...
static bool lora_enqueue_alerta_codigo_controle(uint8_t codigo, int unidade_id, int bomba_id, int controle_id,
                                                int pct) {
